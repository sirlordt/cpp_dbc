...
TEST_CASE("Real Redis connection pool tests", "[24_141_01_redis_real_connection_pool]")
{
    ...
    SECTION("Runtime pool configuration")
    {
        ...
        SECTION("Per-call timeout via getKVDBConnection(timeoutMs)")
        {
            std::vector<std::shared_ptr<cpp_dbc::KVDBConnection>> conns;
            for (size_t i = 0; i < 5; ++i)
            {
            ...
            auto start = std::chrono::steady_clock::now();
            auto result = pool->getKVDBConnection(std::nothrow, 200);
            auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::steady_clock::now() - start);
            ...
            auto result2 = pool->getKVDBConnection(std::nothrow, 3000);
        ...
        SECTION("Connection pool under load with runtime timeout")
        {
            ...
            for (uint64_t i = 0; i < numOperations; i++)
            {
                threads.push_back(std::thread([&pool, &successCount, &failureCount, i]()
                {
                    try
                    {
                        auto loadConn = pool->getKVDBConnection(6000);
                        if (!loadConn)
                        {
                            failureCount++;
                            return;
