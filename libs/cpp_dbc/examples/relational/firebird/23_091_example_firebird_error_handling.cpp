 ...
 *
 * @file 23_091_example_firebird_error_handling.cpp
 * @brief Firebird-specific example demonstrating database error handling
 *
 ...
 * - Firebird-specific error handling (GDSCODE, SQLCODE)
 *
 * Usage:
 *   ./23_091_example_firebird_error_handling [--config=<path>] [--db=<name>] [--help]
 *
 * Exit codes:
 *   0   - Success
 *   1   - Runtime error
...
void demonstrateDataTypeErrors(std::shared_ptr<cpp_dbc::RelationalDBConnection> conn)
{
    ...
    executeWithErrorHandling("Numeric Overflow", [&conn]()
                             {
        auto rs = conn->executeQuery("SELECT CAST(99999999999999999999 AS NUMERIC(10,2)) FROM RDB$DATABASE");
        rs->next(); });
}
...
void demonstrateErrorRecovery(std::shared_ptr<cpp_dbc::RelationalDBConnection> conn)
{
    ...
    catch (const cpp_dbc::DBException &e)
    {
        ...
        try
        {
            ...
            auto rs = conn->executeQuery("SELECT COUNT(*) as cnt FROM error_test_customers");
            rs->next();
            int count = rs->getInt("cnt");
...
void demonstrateFirebirdSpecificErrors(std::shared_ptr<cpp_dbc::RelationalDBConnection> conn)
{
    ...
    logInfo("  - isc_deadlock: Deadlock detected");
    logInfo("  - isc_lock_conflict: Lock conflict on resource");
    logInfo("  - isc_io_error: I/O error during operation");
    logInfo("  - isc_net_connect_err: Unable to complete network request");
}
...
int main(int argc, char *argv[])
{
    ...
    if (args.showHelp)
    {
        printHelp("23_091_example_firebird_error_handling", "firebird");
        return EXIT_OK_;
    }
