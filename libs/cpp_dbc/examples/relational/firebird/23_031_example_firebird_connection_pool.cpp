 ...
 *
 * @file 23_031_example_firebird_connection_pool.cpp
 * @brief Firebird-specific example demonstrating connection pooling
 *
 * This example demonstrates:
 * - Loading configuration from YAML file
 * - Creating a Firebird connection pool
 * - Concurrent multi-threaded database access
 * - Pool statistics monitoring
 * - Proper pool lifecycle management
 *
 * Usage:
 *   ./23_031_example_firebird_connection_pool [--config=<path>] [--db=<name>] [--help]
 *
 * Exit codes:
 *   0   - Success
 *   1   - Runtime error
...
void performDatabaseOperation(cpp_dbc::RelationalDBConnectionPool &pool, int threadId)
{
    ...
    try
    {
        ...
        // Simulate database operation with Firebird-specific query
...
int main(int argc, char *argv[])
{
    ...
    if (args.showHelp)
    {
        printHelp("23_031_example_firebird_connection_pool", "firebird");
        return EXIT_OK_;
    }
    ...
    try
    {
        ...
        logInfo("Max size: 10");
        logInfo("Validation query: SELECT 1 FROM RDB$DATABASE");
