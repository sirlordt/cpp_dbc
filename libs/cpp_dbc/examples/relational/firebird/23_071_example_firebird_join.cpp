 ...
 *
 * @file 23_071_example_firebird_join.cpp
 * @brief Firebird-specific example demonstrating SQL JOIN operations
 *
 * This example demonstrates:
 * - INNER JOIN, LEFT JOIN, RIGHT JOIN
 * - CROSS JOIN, SELF JOIN
 * - FULL OUTER JOIN (supported in Firebird)
 * - JOIN with aggregate functions
 * - Multi-table JOIN and JOIN with subquery
 *
 * Usage:
 *   ./23_071_example_firebird_join [--config=<path>] [--db=<name>] [--help]
 *
 * Exit codes:
 *   0   - Success
 *   1   - Runtime error
...
/**
 * @brief Sets up test database schema and data for JOIN demonstrations
 *
 * FIREBIRD-SPECIFIC TRANSACTION AND RESOURCE MANAGEMENT:
 * =======================================================
 *
 * CRITICAL PATTERN FOR FIREBIRD:
 * -------------------------------
 * 1. Execute all INSERT statements with PreparedStatement
 * 2. CLOSE all PreparedStatement objects (customerStmt, productStmt, orderStmt)
 * 3. COMMIT the transaction to make data visible to subsequent queries
 *
 * WHY COMMIT IS NECESSARY:
 * ------------------------
 * In Firebird, data inserted within a transaction is NOT visible to SELECT queries
 * in the same connection until you COMMIT. Without commit:
 * - SELECT queries will return ZERO rows or NULL values
 * - Aggregate functions (SUM, COUNT) will fail with "Data type unknown"
 * - JOIN operations will show empty or malformed results
 *
 * WHY CLOSE PreparedStatement OBJECTS:
 * ------------------------------------
 * Firebird maintains locks on tables while PreparedStatement objects are active.
 * If not closed explicitly before queries or DDL operations, you'll encounter:
 * - "SQLCODE -607: object TABLE is in use" errors
 * - Metadata lock conflicts
 *
 * ORIGINAL BUG:
 * -------------
 * Initially, this function did NOT commit after inserts. Result:
 * - SELECT queries showed NULL values for order_date and total_price
 * - Aggregate query failed: "SQLCODE -204: Data type unknown"
 * - Cause: Firebird couldn't determine SUM() type on uncommitted NULL data
 *
 * FIX APPLIED:
 * ------------
 * Added customerStmt->close(), productStmt->close(), orderStmt->close()
 * Added conn->commit() to make inserted data visible to subsequent queries
 *
 * This explicit pattern is REQUIRED in Firebird for proper transaction isolation.
 */
void setupDatabase(std::shared_ptr<cpp_dbc::RelationalDBConnection> conn)
{
    ...
    for (const auto &order : orders)
    {
        ...
        orderStmt->setDate(4, std::get<3>(order)); // Use setDate for DATE columns
        orderStmt->setInt(5, std::get<4>(order));
        orderStmt->setDouble(6, std::get<5>(order));
        orderStmt->executeUpdate();
    }

    // Close all prepared statements and commit data
    customerStmt->close();
    productStmt->close();
    orderStmt->close();
    conn->commit(); // Commit all inserts before queries
...
void demonstrateFullOuterJoin(std::shared_ptr<cpp_dbc::RelationalDBConnection> conn)
{
    ...
    std::string query =
        "SELECT c.customer_id, c.name, p.product_id, p.name AS product_name, o.order_id "
        "FROM customers c "
    ...
    printResults(rs);
    rs->close(); // Close result set
    logOk("FULL OUTER JOIN completed");
}
...
void demonstrateCrossJoin(std::shared_ptr<cpp_dbc::RelationalDBConnection> conn)
{
    ...
    std::string query =
        "SELECT FIRST 18 c.customer_id, c.name, p.category "
        "FROM customers c "
...
/**
 * @brief Demonstrates aggregate functions with JOINs
 *
 * FIREBIRD-SPECIFIC REQUIREMENT FOR SUM() WITH LEFT JOIN:
 * ========================================================
 * When using SUM() on a column from a LEFT JOIN where the joined table may be NULL,
 * Firebird cannot determine the return data type during statement preparation, causing:
 * "SQLCODE -204: Data type unknown"
 *
 * SOLUTION: Use CAST to explicitly specify the return type:
 * - INCORRECT: SUM(o.total_price)
 * - CORRECT:   CAST(SUM(o.total_price) AS NUMERIC(10,2))
 *
 * This is required even with COALESCE because Firebird needs to know the type
 * before evaluating the aggregate function.
 */
void demonstrateJoinWithAggregates(std::shared_ptr<cpp_dbc::RelationalDBConnection> conn)
{
    ...
    std::string query =
        ...
        "CAST(SUM(o.total_price) AS NUMERIC(10,2)) AS total_spent "
        "FROM customers c "
        "LEFT JOIN orders o ON c.customer_id = o.customer_id "
...
int main(int argc, char *argv[])
{
    ...
    if (args.showHelp)
    {
        printHelp("23_071_example_firebird_join", "firebird");
        return EXIT_OK_;
    }
