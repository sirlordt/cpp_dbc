 ...
 *
 * @file 21_091_example_postgresql_error_handling.cpp
 * @brief PostgreSQL-specific example demonstrating database error handling
 *
 ...
 * - PostgreSQL-specific error handling (invalid fillfactor, advisory locks)
 *
 * Usage:
 *   ./21_091_example_postgresql_error_handling [--config=<path>] [--db=<name>] [--help]
 *
 * Exit codes:
 *   0   - Success
 *   1   - Runtime error
...
void demonstrateDataTypeErrors(std::shared_ptr<cpp_dbc::RelationalDBConnection> conn)
{
    ...
    executeWithErrorHandling("Type Conversion Error", [&conn]()
                             {
        auto rs = conn->executeQuery("SELECT 'abc'::integer FROM error_test_customers");
        rs->next(); });

    // Invalid date format
    executeWithErrorHandling("Invalid Date Format", [&conn]()
                             { conn->executeQuery("SELECT '2023-13-32'::date"); });

    // Numeric overflow
    executeWithErrorHandling("Numeric Overflow", [&conn]()
                             {
        auto rs = conn->executeQuery("SELECT 9999999999999999999999999999::numeric * 9999999999999999999999999999::numeric FROM error_test_customers");
        rs->next(); });
}
...
void demonstrateConnectionErrors()
{
    ...
    executeWithErrorHandling("Connection Error", []()
                             { auto conn = cpp_dbc::DriverManager::getDBConnection(
                                   "cpp_dbc:postgresql://localhost:5432/nonexistent_db",
                                   "invalid_user",
                                   "invalid_password"); });
...
void demonstratePreparedStatementErrors(std::shared_ptr<cpp_dbc::RelationalDBConnection> conn)
{
    ...
    executeWithErrorHandling("Invalid Parameter Index", [&conn]()
                             {
        auto pstmt = conn->prepareStatement(
            "SELECT * FROM error_test_customers WHERE customer_id = $1"
        );
    ...
    executeWithErrorHandling("Missing Parameter", [&conn]()
                             {
        auto pstmt = conn->prepareStatement(
            "SELECT * FROM error_test_customers WHERE customer_id = $1 AND name = $2"
        );
    ...
    auto executeWithLogging = [&customLogError, &conn](const std::string &sql, const std::string &operation)
    {
        ...
        catch (const cpp_dbc::DBException &e)
        {
            ...
            if (errorMsg.find("constraint") != std::string::npos ||
                ...
                errorMsg.find("violates") != std::string::npos)
            {
...
// Function to demonstrate PostgreSQL-specific errors
void demonstratePostgreSQLSpecificErrors(std::shared_ptr<cpp_dbc::RelationalDBConnection> conn)
{
    logMsg("");
    logMsg("--- PostgreSQL-Specific Error Handling ---");

    // Invalid fillfactor
    executeWithErrorHandling("PostgreSQL Invalid Fillfactor", [&conn]()
                             { conn->executeUpdate(
                                   "CREATE TABLE invalid_table WITH (fillfactor=invalid_value) AS SELECT 1"); });

    // Division by zero (PostgreSQL throws an error unlike MySQL)
    executeWithErrorHandling("PostgreSQL Division by Zero", [&conn]()
                             {
        auto rs = conn->executeQuery("SELECT 1/0 as result");
        rs->next(); });

    // Array type error
    executeWithErrorHandling("PostgreSQL Array Type Error", [&conn]()
                             { conn->executeQuery("SELECT ARRAY[1,2,3]::text[]"); });

    // Invalid regex pattern
    executeWithErrorHandling("PostgreSQL Invalid Regex", [&conn]()
                             { conn->executeQuery("SELECT 'test' ~ '['"); });

    // Exclusion constraint violation (demonstration query)
    executeWithErrorHandling("PostgreSQL Extension Error", [&conn]()
                             {
        // Try to load a non-existent extension
        conn->executeUpdate("CREATE EXTENSION IF NOT EXISTS nonexistent_extension"); });

    logInfo("In production, handle advisory lock conflicts with retry logic");
    logInfo("Consider using pg_try_advisory_lock() for non-blocking lock acquisition");
    logInfo("Handle 'too many connections' errors with proper connection pooling");
}
...
void runAllDemonstrations(std::shared_ptr<cpp_dbc::RelationalDBConnection> conn)
{
    ...
    demonstratePostgreSQLSpecificErrors(conn);
...
{
    logMsg("========================================");
    logMsg("cpp_dbc PostgreSQL Error Handling Example");
    logMsg("========================================");
    logMsg("");
    ...
    if (args.showHelp)
    {
        printHelp("21_091_example_postgresql_error_handling", "postgresql");
        return EXIT_OK_;
    }
