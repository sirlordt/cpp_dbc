 ...
 *
 * @file 21_031_example_postgresql_connection_pool.cpp
 * @brief PostgreSQL-specific example demonstrating connection pooling
 *
 ...
 *
 * Usage:
 *   ./21_031_example_postgresql_connection_pool [--config=<path>] [--db=<name>] [--help]
 *
 * Exit codes:
 *   0   - Success
 *   1   - Runtime error
...
void performDatabaseOperation(cpp_dbc::RelationalDBConnectionPool &pool, int threadId)
{
    ...
    {
        logError("Thread " + std::to_string(threadId) + " error: " + std::string(e.what_s()));
    }
}
#endif
...
int main(int argc, char *argv[])
{
    ...
    if (args.showHelp)
    {
        printHelp("21_031_example_postgresql_connection_pool", "postgresql");
        return EXIT_OK_;
    }
    ...
    auto dbResult = getDbConfig(configManager, args.dbName, "postgresql");
