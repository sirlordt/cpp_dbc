...
namespace cpp_dbc::ScyllaDB
{
    ...
    cpp_dbc::expected<void, DBException> ScyllaDBPreparedStatement::checkSession(std::nothrow_t) noexcept
    {
        if (m_session.expired())
        {
            SCYLLADB_DEBUG("ScyllaDBPreparedStatement::checkSession - Session is closed");
            return cpp_dbc::unexpected(DBException("A2B3C4D5E6F7", "Session is closed", system_utils::captureCallStack()));
        }
        return {};
    }

    cpp_dbc::expected<void, DBException> ScyllaDBPreparedStatement::recreateStatement(std::nothrow_t) noexcept
    {
