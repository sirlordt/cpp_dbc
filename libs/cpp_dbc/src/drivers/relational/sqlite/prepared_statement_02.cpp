...
namespace cpp_dbc::SQLite
{
    ...
    // No try/catch in any setter: all inner calls are nothrow (scoped_lock, member reads,
    // getSQLiteConnection(std::nothrow), sqlite3_bind_* C functions). The only possible
    // throws are death-sentence exceptions (std::bad_alloc from std::string concatenation
    // in error messages, mutex std::system_error) with no meaningful recovery path.
