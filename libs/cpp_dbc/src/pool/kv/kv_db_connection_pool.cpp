...
namespace cpp_dbc
{
    ...
    KVDBConnectionPool::createPooledDBConnection(std::nothrow_t) noexcept
    {
        ...
        catch (const std::bad_weak_ptr &ex)
        {
            ...
            return cpp_dbc::unexpected(DBException("8YUTP0BMR72X",
    ...
    KVDBConnectionPool::getKVDBConnection(std::nothrow_t) noexcept
    {
        ...
        auto conn = std::dynamic_pointer_cast<KVDBConnection>(result.value());
        if (!conn)
        {
            return cpp_dbc::unexpected(DBException("JHNOCJK18TJX",
                                                   "Failed to cast pooled connection to KVDBConnection",
                                                   system_utils::captureCallStack()));
        }
        return conn;
    }
