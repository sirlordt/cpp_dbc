...
- **Nothrow-First Dual-API Architecture — All 7 Drivers:**
  ...
  - Redis-specific (2026-03-29): `RedisDBConnection` uses `SharedConnMutex m_connMutex` (shared_ptr<recursive_mutex>); native handle renamed from `m_context` to `m_conn`; connection-level macros `REDIS_CONNECTION_LOCK_OR_RETURN/THROW/SUCCESS_IF_CLOSED` in `redis_internal.hpp`; `DB_DRIVER_LOCK_GUARD` macro for conditional locking; `REDIS_DEBUG` rewritten to `snprintf` + `logWithTimesMillis()`; `std::from_chars` for all string-to-number; `memory_order_seq_cst` for all atomics; dead try/catch removed; `getInstance(std::nothrow)` checks `m_initFailed`
