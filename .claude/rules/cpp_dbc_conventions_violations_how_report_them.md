...
- [ ] **Child objects reject operations when parent connection is closed**: Every child (PreparedStatement, ResultSet, Blob, Cursor, Collection, Document) must guard every public method checking both `weak_ptr` expiry AND `m_closed`. This applies even to store-result ResultSets (MySQL, PostgreSQL) that only read in-memory data — the lifecycle contract is enforced regardless of mutex source. `*InputStream` is exempt — it has no connection dependency. `MongoDBDocument` is exempt — it is a pure in-memory BSON wrapper with no connection reference or native DB API calls after construction (same rationale as `*InputStream`).
